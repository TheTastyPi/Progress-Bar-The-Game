	...
	switch(n) {
		...
		case 1:
			...
			for (let i = 0; i < 6; i++) {
				...
				id("logBoostAmount"+i).innerHTML = level + "/" + logBoostLimit[i] + " | " + (level == logBoostLimit[i] ? "Infinity" : game.lifetimePoints[1] - Math.floor(game.lifetimePoints[1] / logBoostReq[i]) * logBoostReq[i]);
				let currentBoost;
				switch(i) {
					case 0:
